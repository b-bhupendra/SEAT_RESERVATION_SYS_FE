...
export function Reservations({ selectedOrg, user }: { selectedOrg?: string, user: User }) {
  ...
  const [statusFilter, setStatusFilter] = useState<string>('paid'); // Lock to paid/active by default
  ...
  }, [page, pageSize, search, statusFilter, sortBy, sortOrder, selectedOrg]);
  ...
  const fetchNextSeatSuggestion = async (sub: string) => {
    if (!sub) return;
    try {
      const org = selectedOrg && selectedOrg !== 'All Organizations' ? selectedOrg.split(',')[0].trim() : 'Trisha Library';
      const params = new URLSearchParams({
        organization: org,
        sub_organization: sub
      });
      const data = await api.get<{ seat_number: string }>(`/api/reservations/next-seat?${params.toString()}`);
      form.setValue("seat_number", data.seat_number);
    } catch (e) {
      console.error("Failed to suggest seat number:", e);
    }
  };
  ...
  const onSubmit = async (data: ReservationFormValues) => {
    ...
    try {
      await api.post('/api/reservations', {
        ...
        organization: org,
        sub_organization: data.subsection,
  ...
  const formatDateSafe = (dateStr: string | null | undefined, formatTemplate: string = 'MMM d, yy') => {
    if (!dateStr) return 'N/A';
    try {
      const d = new Date(dateStr);
      return !isNaN(d.getTime()) ? format(d, formatTemplate) : 'N/A';
    } catch {
      return 'N/A';
    }
  };
  ...
  const handleSendNotification = async (res: Reservation) => {
    try {
      await api.post('/api/notifications', { 
        ...
        message: `Payment Reminder: $${res.amount} for Seat ${res.seat_number} in ${res.subsection} (Valid till ${formatDateSafe(res.end_date, 'MMM d, yyyy')}).` 
      });
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-primary">Live Floor Telemetry</h1>
          <p className="text-muted-foreground">Real-time visual filter of currently active and validated reservations.</p>
        </div>
        {user.role !== 'staff' && (
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Assign a seat, specify duration and amount to create a seat reservation.
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Location / Subsection</FormLabel>
                      <Select 
                        onValueChange={(val) => {
                          field.onChange(val);
                          fetchNextSeatSuggestion(val);
                        }} 
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <div className="flex gap-2">
                          <Input placeholder="Auto or Enter Number" {...field} />
                          <Button 
                            type="button" 
                            variant="outline" 
                            size="sm"
                            onClick={() => fetchNextSeatSuggestion(form.getValues("subsection"))}
                          >
                            Suggest
                          </Button>
                        </div>
      ...
      <Card className="border-none shadow-sm shadow-black/5 mt-4">
            ...
            <TableHeader className="bg-muted/50">
              <TableRow>
                ...
                <TableHead className="font-bold text-xs uppercase tracking-wider cursor-pointer hover:text-primary transition-colors" onClick={() => toggleSort('status')}>
                  <div className="flex items-center">Validation Status {getSortIcon('status')}</div>
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {loading ? (
                ...
                reservations.map((res) => (
                  <TableRow key={res.id} className="hover:bg-muted/30 transition-colors group border-b border-dashed">
                      ...
                      <div className="flex items-center gap-3">
                        <Avatar className="h-8 w-8">
                          <AvatarFallback className="bg-primary/10 text-primary text-[10px] font-bold uppercase">
                            {(res.customer_name || 'U').substring(0, 2)}
                          </AvatarFallback>
                        </Avatar>
                        <span className="font-bold">{res.customer_name || 'Unknown'}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex flex-col">
                        <span className="text-xs font-medium">
                          {formatDateSafe(res.start_date)} - {formatDateSafe(res.end_date)}
                        </span>
                    ...
                    <TableCell>
                      <div className="flex flex-col">
                        <span className="font-bold">${(res.amount ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
